...
for(d in c(1,2,3,4)){
  ...
  for(s in 1:5){
    for(l in 1:5){
        ...
        for(j in 1:N){
          ...
          repeat{
            ...
            if(E_max_sort[3] > L_U[E_max_ord[3],3,d] || E_min_sort[1] < L_L[E_min_ord[1],3,d]){
              ...
              if(E_max_sort[2] > L_U[E_max_ord[2],2,d]){c_ind_up[j,E_max_ord[2]] <- 1}
              if(E_max_sort[1] > L_U[E_max_ord[1],1,d]){c_ind_up[j,E_max_ord[1]] <- 1}
              
              if(E_min_sort[1] < L_L[E_min_ord[1],3,d]){c_ind_down[j,E_min_ord[1]] <- 1}
              if(E_min_sort[2] < L_L[E_min_ord[2],2,d]){c_ind_down[j,E_min_ord[2]] <- 1}
              if(E_min_sort[3] < L_L[E_min_ord[3],1,d]){c_ind_down[j,E_min_ord[3]] <- 1}
              
              break
            }else if( E_max_sort[2] > L_U[E_max_ord[2],2,d] || E_min_sort[2] < L_L[E_min_ord[2],2,d]){
              RL[j] <- count
              if(E_max_sort[3] > L_U[E_max_ord[3],3,d]){c_ind_up[j,E_max_ord[3]] <- 1}
              if(E_max_sort[2] > L_U[E_max_ord[2],2,d]){c_ind_up[j,E_max_ord[2]] <- 1}
              if(E_max_sort[1] > L_U[E_max_ord[1],1,d]){c_ind_up[j,E_max_ord[1]] <- 1}
              ...
              break
            }else if( E_max_sort[1] > L_U[E_max_ord[1],1,d] || E_min_sort[3] < L_L[E_min_ord[3],1,d]){
              RL[j] <- count
              if(E_max_sort[3] > L_U[E_max_ord[3],3,d]){c_ind_up[j,E_max_ord[3]] <- 1}
              if(E_max_sort[2] > L_U[E_max_ord[2],2,d]){c_ind_up[j,E_max_ord[2]] <- 1}
              if(E_max_sort[1] > L_U[E_max_ord[1],1,d]){c_ind_up[j,E_max_ord[1]] <- 1}
      ...
      if(s == 1 & l == 1 & o == 1){
        write_csv(c_df ,file=paste("result/one-side-EWMA/Xpc-5-ARL1-all-Holm-rho-5-down.csv",sep = ""),append = TRUE,
                  col_names = TRUE)
      }else{
        write_csv(c_df ,file=paste("result/one-side-EWMA/Xpc-5-ARL1-all-Holm-rho-5-down.csv",sep = ""),append = TRUE)
      }
