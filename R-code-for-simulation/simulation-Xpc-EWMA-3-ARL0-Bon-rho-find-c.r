...
for(d in 2:length(rho123)){
  ...
  for(k in 1:20){
    ...
    Lx4 <- c(2.67,2.92,3.06) ; Lp4 <- c(2.88,3.14,3.29) ; Lc4 <- c(2.85,3.14,3.31)
    L4 <- matrix(rbind(Lx4,Lp4,Lc4),3,3)
    
    L <- array(0,dim = c(3,3,4))
    L[,,1] <- L1
    L[,,2] <- L2
    L[,,3] <- L3
    L[,,4] <- L4
    ...
    for(j in 1:N){
      ...
      repeat{
        ...
        ## Bonferroni
        if(E_sort[1] > L[E_ord[1],3,d] || E_sort[2] > L[E_ord[2],3,d] ||
           E_sort[3] > L[E_ord[3],3,d] ){
    ...
    if(k == 1){
      ...
      names(df) <- c("rho","k","Lx2","Lp2","Lc2","Lx4","Lp4","Lc4","Lx6","Lp6","Lc6",
                     "ARL","time_elapse")
      write_csv(df,file=paste("result/one-side-EWMA/Xpc-ARL0-Bon-rho-find-L-up.csv",sep = ""),append = TRUE,
                col_names = TRUE)
    }else{
      df <- cbind(rho,k,L[1,1,d],L[2,1,d],L[3,1,d],L[1,2,d],L[2,2,d],L[3,2,d],
                  L[1,3,d],L[2,3,d],L[3,3,d],ARL[k], time_elapse[k])
